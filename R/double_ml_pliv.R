...
#' \eqn{Z = m_0(X) + V},
#'
#' with \eqn{E[\zeta|Z,X]=0} and \eqn{E[V|X] = 0}. \eqn{Y} is the outcome
#' variable, \eqn{D} is the policy variable of interest and \eqn{Z} denotes
#' one or multiple instrumental variables. The high-dimensional vector
#' \eqn{X = (X_1, \ldots, X_p)} consists of other confounding covariates,
#' and \eqn{\zeta} and \eqn{V} are stochastic errors.
#'
#' @usage NULL
#'
#' @examples
    ...
    nuisance_est_partialXZ = function(smpls, ...) {
      ...
      data_aux_list = lapply(m_hat$train_preds, function(x) {
        setnafill(data.table(self$data$data_model, "m_hat_on_train" = x),
          fill = -9999.99
        ) # mlr3 does not allow NA's (values are not used)
      })
...
DoubleMLPLIV.partialX = function(
  data,
  ml_l,
  ml_m,
  ml_r,
...
DoubleMLPLIV.partialZ = function(
  data,
  ml_r,
  n_folds = 5,
  n_rep = 1,
...
DoubleMLPLIV.partialXZ = function(
  data,
  ml_l,
  ml_m,
  ml_r,
