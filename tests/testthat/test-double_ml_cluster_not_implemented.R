...
test_that("Not yet implemented cluster features", {
  ...
  dml_plr = DoubleMLPLR$new(dml_data,
                            ml_g, ml_m)
  smpls = dml_plr$smpls
  msg = paste("Externally setting the sample splitting for DoubleML is not",
  ...
  data_multiway = DoubleMLClusterData$new(dt, y_col='Y', d_cols='D',
  ...
  expect_error(DoubleMLPLIV$new(data_multiway,
                                ml_g, ml_m, ml_r), regexp = msg)

  
  msg = paste("No cross-fitting \\(`apply_cross_fitting = False`\\) is not yet",
