...
patrick::with_parameters_test_that("Unit tests for exception handling of PLR:",
  .cases = test_cases,
  {
    ...
    if ((n_folds > 1 & !apply_cross_fitting) |
      ...
      msg = paste0(
        "Multiplier bootstrap has not yet been performed. First call ",
        "bootstrap\\(\\) and then try confint\\(\\) again."
      )
...
test_that("Unit tests for deprecation warnings of PLR", {
  ...
  expect_warning(
    DoubleMLPLR$new(dml_data, ml_l, ml_m,
      score = "IV-type"
    ),
    regexp = msg
  )
