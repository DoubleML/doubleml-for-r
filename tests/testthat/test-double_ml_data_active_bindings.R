...
test_that("x_cols setter", {
  ...
  data_comp <- as.data.frame(dml_data$data_model)[
    , c(c("X1", "X11", "X13"), dml_data$y_col, dml_data$d_cols)
  ]
  ...
  msg <- paste0(
    ...
    "'X14','X15','X16','X17','X18','X19','X20','y','d'\\}"
  )
...
test_that("Tests for use_other_treat_as_covariate", {
  ...
  msg <- paste0(
    "Assertion on 'use_other_treat_as_covariate' failed: Must be of type ",
    "'logical', not 'double'."
  )
...
test_that("Disjoint sets", {
  ...
  msg <- paste0(
    "At least one variable/column is set as covariate \\('x_cols'\\) and as ",
    "a cluster variable \\('cluster_cols'\\)."
  )
  expect_error(
    DoubleMLClusterData$new(dt,
      y_col = "yy",
