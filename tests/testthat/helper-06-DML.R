...
DML = function(data, y, d, model = "plr", k = 2, S = 1, smpls = NULL,
  ...
  for (s in seq(S)) {
    ...
    res[[s]] = list(
      coefficients = coefficients, se = se, t = t, pval = pval,
...
bootstrap.DML = function(object, data, y, d,
  ...
  n_obs = nrow(data)
  for (s in seq(n_rep)) {
    weights = draw_bootstrap_weights(bootstrap, nRep, n_obs)
    for (j in seq(p1)) {
      ind_start = ((s - 1) * nRep + 1)
      ind_end = (s * nRep)
      boot_theta[j, ind_start:ind_end] = dml_plr_boot(data,
        y = y, d = d[j],
